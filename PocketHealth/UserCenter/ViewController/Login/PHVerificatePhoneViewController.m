//
//  PHVerificatePhoneViewController.m
//  PocketHealth
//
...
#import "PHFindPasswordSettingViewController.h"

@interface PHVerificatePhoneViewController ()<UITextFieldDelegate,PHFindPasswordSettingViewDelegate>
{
    NSTimer *timer;
    NSInteger seconds;
}
...
@implementation PHVerificatePhoneViewController
...
{
    [[PHUpdateUserInfoManager defaultManager] sendPhoneCodeWithTel:tel WithType:FindPassword CallDoneBack:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        [CommonUtil HUDShowText:@"发送失败！" InView:self.view];
    }];
}

-(void)verificatePhone:(NSString *)phone WithVerificateCode:(NSString *)code
{
    [[PHUpdateUserInfoManager defaultManager] verificateUserWithPhone:phone WithVerificateCode:code WithUserId:0  CallDoneBack:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSDictionary *dic = (NSDictionary *)responseObject;
        NSInteger code = [[dic objectForKey:@"Code"] integerValue];
        if (code == 1) {
...
//                [[PHAppStartManager defaultManager] setHostMember:host];
                ...
                PHFindPasswordSettingViewController *phfindPasswordSettingVC = [[PHFindPasswordSettingViewController alloc] init];
                phfindPasswordSettingVC.delegate = self;
                [self.navigationController pushViewController:phfindPasswordSettingVC animated:YES];
            }
        }else{
            [CommonUtil HUDShowText:[dic objectForKey:@"Message"] InView:self.view];
        }
