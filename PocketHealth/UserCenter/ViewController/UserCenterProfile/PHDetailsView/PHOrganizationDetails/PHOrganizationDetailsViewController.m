...
typedef void (^tmpResultFlag)(BOOL);
@interface PHOrganizationDetailsViewController ()<PHDetailsOtherInfoDelegate,PHInterestingGroupDelegate,UIAlertViewDelegate>
{
    ...
    long long tmpIOS7_orgUserid;
    tmpResultFlag callbackResult;
}
...
-(void)dontAttentionOrganization:(Organization *)org result:(void (^)(BOOL))resultFlag
{
    ...
    if (IOS_VERSION_8_OR_ABOVE) {
        ...
        UIAlertView *alertViewIOS7 = [[UIAlertView alloc]initWithTitle:@"是否取消关注" message:nil
                                                              delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
        tmpIOS7_orgUserid = org.userId;
        
        callbackResult = resultFlag;
        [alertViewIOS7 show];
    }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1) {
        [self.delegate deleteFriendWithMemberId:tmpIOS7_orgUserid result:^(BOOL flag) {
            //            NSLog(@"%hhd",flag);
            callbackResult(flag);
            //                if (flag) {
            //                    [filterOrganizationList removeObject:selectOrganization];
