...
#import "CustomDefultTableViewCell.h"
@interface GroupChatGroupMemberListTableViewController ()<PHGroupPersonalInfoViewControllerDelegate>
{
    Group * group;
...
static NSString * indenrifierGroupMemberList = @"indenrifierGroupMemberList";
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"CustomDefultTableViewCell" bundle:nil] forCellReuseIdentifier:indenrifierGroupMemberList];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CustomDefultTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:indenrifierGroupMemberList];
    if (!cell) {
        cell = [[CustomDefultTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:indenrifierGroupMemberList];
    }
    // Configure the cell...
    GroupMember *gmember ;
    ...
    if (gmember) {
        cell.cunstomLabel.text = gmember.nickName;
        [cell.customImageView sd_setImageWithURL:[NSURL URLWithString:gmember.headImage]];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    for(GroupMember *gMember in group.groupMember)
    {
        if (gMember.memberId == host.memberId) {
            groupHost = gMember;
            break;
        }
    }
    for(GroupMember *gsendMember in group.groupMember)
    {
        if (gsendMember.memberId == gmember.memberId) {
            gmember = gsendMember;
            break;
        }
    }
    PHGroupPersonalInfoViewController * groupPersonalVC = [[PHGroupPersonalInfoViewController alloc]initWithGroupMember:gmember WithMyGroupHost:groupHost];
    groupPersonalVC.delegate = self;
    [self.navigationController pushViewController:groupPersonalVC animated:YES];
}

#pragma mark - 委托方法 删除选定用户
-(void)deleteSelectedMember:(GroupMember *)selectMember{
    [group.groupMember removeObject:selectMember];
