...
+(void)requestNearByHospitalWithLat:(double)lat lon:(double)lon done:(ApiDoneCallback)doneCallback error:(ApiErrorCallback)errorCallback{
    //PH_NearBy_HospitalList
    NSString *urlStr=[NSString stringWithFormat:PH_NearBy_HospitalList];
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    NSString *httpKey = [[[PHAppStartManager defaultManager] userHost]getHttpKey];
    NSNumber * latNum = [NSNumber numberWithDouble:lat];
    NSNumber * lonNum = [NSNumber numberWithDouble:lon];
//    NSString * latStr =[NSString stringWithFormat:@"%f",lon];
//    NSString * lonStr = [NSString stringWithFormat:@"%f",lat];
    NSDictionary *parameters =[[NSDictionary alloc]initWithObjectsAndKeys:httpKey,@"key",lonNum,@"lon",latNum,@"lat",nil];
    [manager POST:urlStr  parameters:parameters success: doneCallback failure:errorCallback];
}
//申请城市列表
...
+(void)adminAcptGroupUserWithRecordid:(unsigned)rid toDo:(joinToDoType)joinat done:(ApiDoneCallback)doneCallback error:(ApiErrorCallback)errorCallback{
    NSString *urlStr=[NSString stringWithFormat:PH_AllowJoinGroups];
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    NSString *httpKey = [[[PHAppStartManager defaultManager] userHost]getHttpKey];
    NSMutableDictionary *parameters =[[NSMutableDictionary alloc]initWithObjectsAndKeys:httpKey,@"key", nil];
    [parameters setObject:[NSString stringWithFormat:@"%u",rid] forKey:@"recordid"];
    [parameters setObject:[NSString stringWithFormat:@"%u",joinat] forKey:@"at"];
    [manager POST:urlStr  parameters:parameters success: doneCallback failure:errorCallback];
}
//热门群分类获取
 ...
 *  @param memberId      操作用户
 *  @param gid           群
 *  @param oType         1 设置管理员 2 取消
 *  @param doneCallback  成功回掉
...
+(void)settingGroupAdminOperateId:(long long)memberId WithGroupId:(unsigned)gid OperateType:(unsigned)oType done:(ApiDoneCallback)doneCallback error:(ApiErrorCallback)errorCallback{
    NSString *urlStr=[NSString stringWithFormat:PH_Setting_Admin];
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    NSString *httpKey = [[[PHAppStartManager defaultManager] userHost]getHttpKey];
    NSMutableDictionary *parameters =[[NSMutableDictionary alloc]initWithObjectsAndKeys:httpKey,@"key", nil];
    [parameters setObject:[NSString stringWithFormat:@"%lld",memberId] forKey:@"toid"];
    [parameters setObject:[NSString stringWithFormat:@"%u",gid] forKeyedSubscript:@"gid"];
    [parameters setObject:[NSString stringWithFormat:@"%u",oType] forKeyedSubscript:@"type"];
    [manager POST:urlStr  parameters:parameters success: doneCallback failure:errorCallback];
}
@end
