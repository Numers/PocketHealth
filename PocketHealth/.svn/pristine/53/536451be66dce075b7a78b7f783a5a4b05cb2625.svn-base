...
-(void)startSocketWithMemberId:(long long )memberId{
    ...
    socketAVDistributer = [XGPSocketAVideoHelper defaultManager];
...
-(void)OnKickNotify:(NSNotification *)notification{
    NSLog(@"OnKickNotify");
    NSDictionary *dic = [notification object];
    [self performSelectorOnMainThread:@selector(loginOutBeKicked:) withObject:dic waitUntilDone:NO];
  
    
}
-(void)loginOutBeKicked:(NSDictionary *)dic{
    NSLog(@"%@",dic);
    if (dic) {
        ...
        if (code==-1) {
            ...
            [CommonUtil HUDShowText:@"你被强制下线" InView:[[UIApplication sharedApplication] keyWindow]];
