...
-(BOOL)validateTextField
{
    ...
    if ((_txtLoginName.text == nil) || (_txtLoginName.text.length == 0)) {
        alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"手机号不能为空!" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
        return NO;
    }
    
    if ((_txtPassword.text == nil) || (_txtPassword.text.length == 0)) {
        alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"密码不能为空!" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
        return NO;
    }
...
-(void)autoLogin
{
    ...
    [[PHLoginManager defaultManager] loginWithLoginName:host.loginName WithPassword:host.passWord WithUdid:udid  WithChanneid:pushChannelId WithBuserid:pushUserId WithDeviceType:Device_IOS needMd5:NO CallDoneBack:^(AFHTTPRequestOperation *operation, id responseObject) {
...
- (IBAction)clickLoginBtn:(id)sender {
    ...
    NSString *pushChannelId = [CommonUtil localUserDefaultsForKey:KMY_PUSH_CHANNELID];
    NSString *udid = [OpenUDID valueWithError:nil];
    [[PHLoginManager defaultManager] loginWithLoginName:_txtLoginName.text WithPassword:_txtPassword.text WithUdid:udid  WithChanneid:pushChannelId WithBuserid:pushUserId WithDeviceType:Device_IOS needMd5:YES CallDoneBack:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSDictionary *dic = (NSDictionary *)responseObject;
        NSInteger code = [[dic objectForKey:@"Code"] integerValue];
        if (code == 1) {
            NSDictionary *resultDic = [dic objectForKey:@"Result"];
