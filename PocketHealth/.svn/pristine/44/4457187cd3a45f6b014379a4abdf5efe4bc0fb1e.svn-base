//
//  PHChatHomeAddDetailMemberViewController.m
//  PocketHealth
//
...
@interface PHChatHomeAddDetailMemberViewController ()<UITableViewDataSource,UITableViewDelegate,UIAlertViewDelegate>
{
    Member *host;
    NSMutableArray * filterHospitalArray;
    NSMutableArray * filterDoctorArray;
    NSMutableArray * filterUserArray;
}
@property (strong, nonatomic) UITableView *tableViewMain;

@end

@implementation PHChatHomeAddDetailMemberViewController
...
- (void)viewDidLoad {
    ...
    filterDoctorArray = [[NSMutableArray alloc]init];
    filterHospitalArray = [[NSMutableArray alloc]init];
    filterUserArray = [[NSMutableArray alloc]init];
    //0.初始化tableview
    ...
    if (IOS_VERSION_8_OR_ABOVE) {
        ...
        frame.size.height -= self.tableViewY;
    }else{
...
//            [filterDoctorArray addObject:member];
//        }
//    }else if (memberUserType == MemberUserTypeAdmin){
//        //医生
//        for (NSDictionary * memberDic in sendResultArray) {
//            NSError *error ;
//            Member * member = [MTLJSONAdapter modelOfClass:[Member class] fromJSONDictionary:memberDic error:&error];
//            [self.resultArray addObject:member];
//            [filterHospitalArray addObject:member];
//        }
//    }
    for (NSDictionary * memberDic in sendResultArray) {
        NSError *error ;
        ...
        if (member.userType == MemberUserTypeDoctor) {
            [filterDoctorArray addObject:member];
        }else if (member.userType == MemberUserTypeAdmin){
            [filterHospitalArray addObject:member];
        }else if (member.userType == MemberUserTypeUser){
            [filterUserArray addObject:member];
        }
    }
...
-(NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
    NSInteger i = 3;
//    if (filterDoctorArray.count > 0) i++;
//    if (filterHospitalArray.count > 0) i++;
//    if (filterUserArray.count) i++;
    return i;
}
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    switch (section) {
        case 0:{
//            if (filterHospitalArray.count==0) {
//                return filterDoctorArray.count;
//            }else{
//                return filterHospitalArray.count;
//            }
            return filterHospitalArray.count == 0?0:filterHospitalArray.count;
        ...
        case 1:{
//            return filterDoctorArray.count;
            return filterDoctorArray.count == 0?0:filterDoctorArray.count;
//            return filterHospitalArray.count;
        }
            break;
        case 2:{
             return filterUserArray.count == 0?0:filterUserArray.count;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.section) {
        ...
        {
            if (filterHospitalArray.count > 0) {
                id inCellMember = filterHospitalArray[indexPath.row];
                [self createCell:inCellMember WithCell:cell WithIndex:indexPath];
            }
            

//            if (filterHospitalArray.count == 0) {
//                id inCellMember = filterDoctorArray[indexPath.row];
//                [self createCell:inCellMember WithCell:cell WithIndex:indexPath];
//            }else{
//                id inCellMember = filterHospitalArray[indexPath.row];
//                [self createCell:inCellMember WithCell:cell WithIndex:indexPath];
//            }
        ...
        {
            if (filterUserArray.count > 0) {
                id inCellMember = filterUserArray[indexPath.row];
                [self createCell:inCellMember WithCell:cell WithIndex:indexPath];
            }
...
-(void)createCell:(id)inCellMember WithCell:(CustomSubtitleHasRightBtnTableViewCell *)cell WithIndex:(NSIndexPath *)indexPath{
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.section) {
        case 0:
        {
            inCellMember = filterHospitalArray[indexPath.row];
        }
            break;
        case 1:{
            inCellMember = filterDoctorArray[indexPath.row];
        }
            break;
        case 2:{
            inCellMember = filterUserArray[indexPath.row];
        }
            break;
        default:
            break;
...
-(void)removeAllUserInResultArray{
    [self.resultArray removeAllObjects];
    [filterDoctorArray removeAllObjects];
    [filterHospitalArray removeAllObjects];
    [filterUserArray removeAllObjects];
}
/*
