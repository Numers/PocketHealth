...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(scrollDraggingGroup:) name:@"SCROLLDRAGGING" object:nil];
        ...
        if (!error) {
            ...
            if([CommonUtil isValidateHttpResponseObject:responseObject]){
                ...
                if (path != nil) {
                    ...
                    retNormal=[ClientHelper sendGroupMessage:self.hostMember.memberId Token:[CommonUtil MyToken] Gid:self.chatToGroup.groupId Message:sendContent Msgsn:message.MsgSN];
...
//        self.chatToGroup.groupMember = [groupMemberdb selectAllGroupMemberWithGid:self.chatToGroup.groupId];
//    });
    
    
    [PHGroupHttpRequest requestAllGroupUsersWithGid:self.chatToGroup.groupId done:^(AFHTTPRequestOperation *operation, id responseObject) {
        //
        
        if ([CommonUtil isValidateHttpResponseObject:responseObject]) {
            NSDictionary * dic = (NSDictionary *)responseObject;
            //更新group数据库
            [PHGroupHttpRequest mergeGroupAndGroupMemberDBWithDic:dic WithBeforeGroup:self.chatToGroup WithMemberID:self.hostMember.memberId];
...
//            if (!error) {
//                self.chatToGroup.groupHeadImage = tmpGroup.groupHeadImage;
//            }
//            SGroupDB * groupDB = [[SGroupDB alloc]init];
//            if (![groupDB isExistGroupWithGid:tmpGroup.groupId WithBelongUid:self.hostMember.memberId]) {
...
//                
//            }
//            self.chatToGroup.groupMember = newGroupMemberList;
...
{
    [self.view endEditing:YES];
    [self messageViewAnimationWithMessageRect_scrollerDrag:CGRectZero
                           withMessageInputViewRect:self.g_messageToolView.frame
                                        andDuration:g_animationDuration
                                           andState:ZBMessageViewStateShowNone];
}
