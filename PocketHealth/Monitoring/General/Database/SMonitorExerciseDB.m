...
//    }
//    return steps;
    NSString * query =[NSString stringWithFormat:@"SELECT  sum(steps)allsteps FROM ExerciseDB WHERE belongMemberId = '%lld' and belongDateTime like '%@%%' ",hostId,belongDateTime];
    FMResultSet * rs = [_db executeQuery:query];
    //    NSMutableArray * array = [NSMutableArray arrayWithCapacity:[rs columnCount]];
    NSInteger steps = 0;
    if ([rs next]) {
        steps = [rs intForColumn:@"allsteps"];
    }
    return steps;
