...
//    NSLog(@"%@",notifiMessage);
    ...
    notifiMessage.state = NotificationMessageStateUnHandle;
    resultBool =  [notifiMsgDB saveNoticationMessage:notifiMessage HostId:myid];
    
    if (resultBool) {
        //3.发送通知 告知UI更新 (如果保存成功)
        [[NSNotificationCenter defaultCenter]postNotificationName:@"DBTOUIJOINCHATORGROUPINFO" object:nil];
    }
    return resultBool;
}
...
bool handlePrivateChatContentMessage(void *pData){
    ...
    //视频消息
    if (myid != uid && msgCT == MessageContentAVStart) {
        //告知help类有视频请求进来
        [[XGPSocketAVideoHelper defaultManager]getOneAVRequestWithInfoDic:dic];
        return YES;
    }
    if (msgCT == MessageContentAVClose || msgCT == MessageContentAVBusyClose) {
        //告知help类有视频请求进来
        
        [[XGPSocketAVideoHelper defaultManager]getOneAVRequestFinish:dic];
        return YES;
    }
